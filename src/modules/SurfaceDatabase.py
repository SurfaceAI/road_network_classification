...
class SurfaceDatabase:
    def __init__(self, dbname, dbuser, dbhost, dbpassword, 
                 pbf_path, alt_road_network=None):
    ...
    def table_to_shapefile(self, table_name, output_file):
        subprocess.run(f'pgsql2shp -f "{output_file}" {self.dbname} "select * from {table_name}"', shell = True, executable="/bin/bash")
    ...
    def add_img_metadata_table(self, aoi, mapillary_interface):
        ...
        self.execute_sql_query(query_file,
    ...
    def add_model_results(self, aoi):
        ...
        pred = utils.format_predictions(pd.read_csv(aoi.pred_path, 
                                                    dtype={"Image": str, "Level_1":str}), 
        ...
        query_file = const.SQL_JOIN_MODEL_PRED_DIR if aoi.use_pano else const.SQL_JOIN_MODEL_PRED

        self.execute_sql_query(query_file, 
                        {"table_name_point_selection": f"{aoi.name}_img_metadata",
                        "csv_path": csv_path})

        query_file = const.SQL_JOIN_TYPE_PRED if not aoi.custom_road_type_join else aoi.custom_road_type_join

        if aoi.road_type_pred_path:
            logging.info("add scenery results to db")
            pred_scene = utils.format_scenery_predictions(pd.read_csv(aoi.road_type_pred_path, 
                                                                      dtype={"Image": str}), 
                                                                      is_pano=aoi.use_pano)
    ...
    def roadtype_separation(self, aoi):
        query_file = const.SQL_ASSIGN_ROAD_TYPES if not aoi.custom_road_type_separation else aoi.custom_road_type_separation
        if query_file != None:
    ...
    def process_area_of_interest(self, area_of_interest, mapillary_interface):
        ...
        output_file = os.path.join(area_of_interest.data_path, area_of_interest.run, f"{area_of_interest.name}_pred.shp")
        self.table_to_shapefile(f"{area_of_interest.name}_group_predictions", output_file)
